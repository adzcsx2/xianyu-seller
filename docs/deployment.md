...
- 自动 Token 刷新默认使用 `SLIDER_HEADLESS=true`；如需有头模式，设置为 `false`，并确保 `DISPLAY` 可用。Docker 镜像内置 Xvfb，可用于提供虚拟显示。
- 浏览器运行时优先使用固定版本的 `slidex`，缺失时回退本地兼容实现；底层仍优先使用 Patchright（Playwright 的反检测分支）。
- 拖动轨迹按设计时间轴回放，机器性能不足时自动降采样，避免轨迹被拉长而失真。
- 全局同时只允许一个浏览器实例，避免多账号同时验证互相拖慢。
- 自动验证失败后立即熔断并进入冷却，不会反复重试——继续重试只会延长风控时间。
- 只有取得真实 `x5sec` Cookie 才判定验证成功；仅视觉上拖动完成、只返回 `x5secdata`/`x5sectag` 或未返回 Cookie 时不会更新账号会话。
...
```bash
docker compose logs xianyu-app | grep -E "Xvfb|Patchright|slidex|滑块引擎"
```
...
- 应用日志按天轮转，文件日志、验证码日志和测试产物统一保留最近 7 天；超过期限的直接文件或测试产物目录会在启动/运行时清理。
- 「通知与日志 → 系统日志」仅管理员可见，支持时间、级别、来源筛选和向前分页；首批最多 1000 条，页面默认每 30 秒刷新。
- 「清空日志」会清空磁盘日志和内存缓冲，且不可恢复；操作前应先导出或复制必要的脱敏诊断信息。
- 测试命令产生的文件都位于 `logs/test-artifacts/`，不会覆盖版本库中的 `static/`；不要将 Cookie、Token、买家消息、卡密或知识库正文写入测试产物。
...
6. 在「知识库」上传 UTF-8 Markdown/TXT，预览后只导入确认可作为买家可见事实的章节，并在问答区核对本次依据。
7. 在「自动发货」中为指定商品绑定卡券，或设置账号级、全局关键词规则。
8. 在「订单」和「消息管理」页面核对实际触发结果。
9. 在「通知与日志」中排查过滤、暂停、无规则、发送失败和监听退出。
...
- AI 模型服务地址只接受 `http` 或 `https`，不允许携带用户名、密码、查询串或片段；普通 AI 请求会拒绝解析到本机、内网、链路本地、保留、多播或未指定地址，也不跟随重定向。托管运行时将公网 DNS 结果映射到 `198.18.0.0/15` 时，仅对 DNS 主机名放行该映射，字面量 IP 仍会被拒绝。
- `/ai-models` 的系统级模型列表只允许管理员调用；账号级列表仍按账号归属校验。AI 配置接口默认只返回密钥是否已配置，只有受保护的管理员设置响应才可查看部署级密钥。
